...
import com.bigdatalighter.kafka.utils.ConfigUtils;
import com.bigdatalighter.kafka.utils.DatasetFilterUtils;
...
public class KafkaWrapper implements Closeable {
    ...
    public static final String USE_NEW_KAFKA_API = "use.new.kafka.api";
    ...
    public static class Builder {
        ...
        public Builder withBrokers(String brokers) {
            String[] bs = brokers.split(",");
            return withBrokers(Arrays.asList(bs));
        }

        public Builder withBrokers(List<String> brokers) {
    ...
    public KafkaWrapper(Builder builder) {
    ...
    public static KafkaWrapper create(Config config) {
        Preconditions.checkArgument(config.hasPath(KAFKA_BROKERS), "Need to specify at least one Kafka broker.");
        Builder builder = new Builder();
        if (ConfigUtils.getBoolean(config, USE_NEW_KAFKA_API, DEFAULT_USE_NEW_KAFKA_API)) {
            builder = builder.withNewKafkaAPI();
        }
        return builder.withBrokers(ConfigUtils.getStringList(config,KAFKA_BROKERS))
                .withConfig(config)
                .build();
    }
    ...
    private class KafkaOldAPI extends KafkaAPI {
        ...
        public static final String CONFIG_KAFKA_CLIENT_NAME_DEFAULT = "temp-kafka-reader";
            ...
            for (int i = 0; i < this.fetchOffsetRetries; i++) {
                try {
                    ...
                    if (offsetResponse.hasError()) {
                        throw new RuntimeException(
                                "offsetResponse has error: " + offsetResponse.errorCode(partition.getTopicName(), partition.getId()));
                    }
